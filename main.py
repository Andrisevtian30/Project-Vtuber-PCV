...
# --- KONFIGURASI UTAMA ---
OSC_IP = "192.168.1.8"   # ganti sesuai IPv4 LAN
OSC_PORT = 39539
WEBCAM_ID = 0
TARGET_FPS = 30
...
def euler_to_quaternion(pitch, yaw, roll):
    """Convert Euler angles (pitch, yaw, roll) to quaternion [qx, qy, qz, qw]"""
...
def get_finger_quat(angle, axis_idx):
    """Generate quaternion rotation for finger curl based on specified axis (0=X, 1=Y, 2=Z)"""
    s = math.sin(angle / 2)
    c = math.cos(angle / 2)
    if axis_idx == 0:   return [s, 0, 0, c]  # X-axis
    elif axis_idx == 1: return [0, s, 0, c]  # Y-axis
    elif axis_idx == 2: return [0, 0, s, c]  # Z-axis
    return [0, 0, 0, 1]  # Default (no rotation)

def get_limb_rotation(start, end, rest_vector):
    """Calculate limb rotation quaternion from start to end point relative to rest pose"""
...
def get_relative_iris(face_landmarks, iris_idx, inner_idx, outer_idx, img_w, img_h):
    """Calculate normalized iris position relative to eye corners for gaze tracking"""
...
def get_finger_curl(landmarks, tip_idx, knuckle_idx, wrist_idx):
    """Calculate finger curl value (0.0=extended, 1.0=fully curled) based on tip-to-wrist distance"""
...
# --- CLASS STABILIZER ---
class Stabilizer:
    """Kalman filter-based stabilizer for smoothing tracking data"""
    def __init__(self, state_num=2, measure_num=1,
                 cov_process=0.0001, cov_measure=0.1):
...
print(f"Streaming to OSC -> {OSC_IP}:{OSC_PORT}")
